"""Compare OCR readability for LR, Real-ESRGAN and HR TextZoom images.
...
SR_CSV = Path("experiments/metrics/easyocr_realesrgan_300_results.csv")
LABELS_CSV = Path("datasets/textzoom_subset_300/textzoom_subset_300_labels.csv")

CONFIDENCE_OUTPUT_CSV = Path(
    "experiments/metrics/ocr_confidence_summary_300.csv"
)
READABILITY_OUTPUT_CSV = Path(
    "experiments/metrics/ocr_readability_summary_300.csv"
)
...
def main() -> None:
    labels = read_labels()
    rows = read_ocr_rows(LR_HR_CSV) + read_ocr_rows(SR_CSV)
